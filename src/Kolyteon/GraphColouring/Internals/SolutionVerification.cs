...
internal static class SolutionVerification
{
    ...
    {
        internal override Result VerifyCorrect(IReadOnlyDictionary<Node, Colour> solution, GraphColouringProblem problem)
        {
    ...
    {
        internal override Result VerifyCorrect(IReadOnlyDictionary<Node, Colour> solution,
            GraphColouringProblem problem) => problem
            .NodeData.Select(datum => datum.Node)
            .Where(node => !solution.ContainsKey(node))
            .Select(node => Result.Failure($"Node {node} is not a key in the solution."))
    ...
    private sealed class EveryNodeHasPermittedColourVerifier : GraphColouringSolutionVerifier
    {
        internal override Result VerifyCorrect(IReadOnlyDictionary<Node, Colour> solution,
            ...
            .Select(item => Result.Failure($"Node {item.Node} is assigned the colour '{item.AssignedColour}', " +
    ...
    private sealed class NoAdjacentNodesSameColourVerifier : GraphColouringSolutionVerifier
    {
        internal override Result VerifyCorrect(IReadOnlyDictionary<Node, Colour> solution,
            ...
            .Select(item => Result.Failure($"Adjacent nodes {item.FirstNode} and {item.SecondNode} " +
