...
/// <summary>
///     Unit tests for the internal <see cref="BTStrategy{V,D}" /> class, parametrized over the Map Colouring problem
///     types.
/// </summary>
public sealed class BTStrategyTests
{
    ...
    private static Mock<IOrderingStrategy> MockOrderingStrategyThatNeverReorders()
    {
        ...
        mock.Setup(m => m.GetLevelOfOptimalNode(It.IsAny<IList<BTNode<Region, Colour>>>(), It.IsAny<int>()))
    ...
    {
        [Fact]
        public void Is_Backtracking_SearchEnumValue()
        {
            // Arrange
            BTStrategy<Region, Colour> sut = new(0);

            // Assert
            sut.Identifier.Should().Be(Search.Backtracking);
        }
    }
    ...
    public sealed class StateTransitions
    {
        ...
        {
            // Arrange
            BTStrategy<Region, Colour> sut = new(3);
        ...
        public void StateIsUnsafe_BacktrackInvoked_SearchLevelIsEarlierNonLeafLevel_CandidatesNotExhausted_StateIsSafe()
        {
            ...
            using (new AssertionScope())
            {
                sut.SearchState.Should().Be(SearchState.Unsafe);
                sut.SearchLevel.Should().Be(2);
                mockOrderingStrategy.Verify(s => s.GetLevelOfOptimalNode(sut.SearchTree, 0));
                mockOrderingStrategy.Verify(s => s.GetLevelOfOptimalNode(sut.SearchTree, 1));
                mockOrderingStrategy.Verify(s => s.GetLevelOfOptimalNode(sut.SearchTree, 2));
                mockOrderingStrategy.VerifyNoOtherCalls();
                sut.SearchTree.Should().SatisfyRespectively(at0 =>
                {
                    at0.VariableIndex.Should().Be(0);
    ...
    public sealed class GetAssignments_Method
    {
        ...
        {
            // Arrange
            BTStrategy<Region, Colour> sut = new(1);
