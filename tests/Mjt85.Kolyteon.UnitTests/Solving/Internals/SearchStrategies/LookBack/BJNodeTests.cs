...
/// <summary>
///     Unit tests for the internal <see cref="BJNode{V,D}" /> class, parametrized over the Map Colouring problem types.
/// </summary>
public sealed class BJNodeTests
{
    ...
    private static BJNodeBuilder GetNode() => new();

    private class BJNodeBuilder
    {
        private ISolvableBinaryCsp<Region, Colour>? _binaryCsp;
        private int _variableIndex;

        public BJNodeBuilder WithBinaryCsp(ISolvableBinaryCsp<Region, Colour> binaryCsp)
        {
        ...
        public BJNodeBuilder WithVariableIndex(int index)
        {
        ...
        public BJNode<Region, Colour> Build() => new(_binaryCsp!, _variableIndex);
    }
    ...
    public sealed class Candidates_Property
    {
        ...
        {
            // Arrange
            BJNode<Region, Colour> sut = GetNode()
    ...
    public sealed class AdjacentTo_Method
    {
        ...
        public void InstanceAndOtherHaveAdjacentVariables_ReturnsTrue()
        {
            ...
            BJNode<Region, Colour> nodeAt0 = GetNode().WithBinaryCsp(binaryCsp).WithVariableIndex(0).Build();
            BJNode<Region, Colour> nodeAt1 = GetNode().WithBinaryCsp(binaryCsp).WithVariableIndex(1).Build();
    ...
    public sealed class AssignNextCandidate_Method
    {
        ...
        {
            // Arrange
            BJNode<Region, Colour> sut = GetNode().WithBinaryCsp(GetBinaryCsp.ModellingProblem(MapColouringPuzzle.Create()
                    .WithGlobalColours(Colour.Black, Colour.White)
                    .AddRegion(R0)
                    .Build()))
                .WithVariableIndex(0)
    ...
    public sealed class RepopulateAncestors_Method
    {
        ...
        public void AddsAllAdjacentNodesAtEarlierSearchTreeLevelsToAncestors_OrderedBySearchTreeLevel()
        {
            ...
            BJNode<Region, Colour> nodeAt0 = GetNode().WithBinaryCsp(binaryCsp).WithVariableIndex(0).Build();
            BJNode<Region, Colour> nodeAt1 = GetNode().WithBinaryCsp(binaryCsp).WithVariableIndex(1).Build();
            BJNode<Region, Colour> nodeAt2 = GetNode().WithBinaryCsp(binaryCsp).WithVariableIndex(2).Build();
            BJNode<Region, Colour> nodeAt3 = GetNode().WithBinaryCsp(binaryCsp).WithVariableIndex(3).Build();
            BJNode<Region, Colour> nodeAt4 = GetNode().WithBinaryCsp(binaryCsp).WithVariableIndex(4).Build();

            BJNode<Region, Colour>[] searchTree = [nodeAt0, nodeAt1, nodeAt2, nodeAt3, nodeAt4];
    ...
    [UnitTest]
    public sealed class SetBacktrackLevelToMax_Method
    {
        [Fact]
        public void SetsBacktrackLevelToOneLessThanSearchTreeLevel()
        {
            // Arrange
            BJNode<Region, Colour> sut = GetNode().WithBinaryCsp(GetBinaryCsp.ModellingProblem(MapColouringPuzzle.Create()
                    .WithGlobalColours(Colour.Black)
